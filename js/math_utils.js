...
/**
 * Valida si una expresión matemática contiene solo las variables permitidas y es sintácticamente correcta.
 * @param {String} exprStr - La expresión a validar
 * @param {Array<String>} variablesPermitidas - Lista de variables permitidas (ej: ['x', 'y'])
 * @returns {Object} { valido: boolean, error: string|null }
 */
window.MathUtils.validarExpresion = function (exprStr, variablesPermitidas) {
    ...
    try {
        ...
        // Recorrer el árbol de nodos buscando símbolos (variables)
        const simbolosEncontrados = new Set();
        node.traverse(function (node, path, parent) {
            if (node.isSymbolNode) {
                // Ignorar constantes matemáticas comunes si math.js las trata como símbolos a veces
                // Pero math.js suele saber que 'pi' es constante. 
                // Sin embargo, verificamos si es una función conocida (sin, cos, etc) o constante.
                // En math.js, los nombres de funciones también aparecen como SymbolNodes si no se ejecutan? 
                // traverse visita todo.

                // Una forma segura es ver si está en variablesPermitidas
                // O si existe en el scope de math (funciones/constantes)
                // Pero cuidado, si el usuario escribe 'z', math.js no lo conoce por defecto como función.

                simbolosEncontrados.add(node.name);
            }
        });

        // Filtrar símbolos que NO son permitidos Y NO son funciones/constantes de math.js
        const invalidos = [];
        simbolosEncontrados.forEach(simbolo => {
            if (variablesPermitidas.includes(simbolo)) return; // Es variable permitida

            // Chequear si es algo de math.js (pi, e, sin, cos, etc.)
            // m[simbolo] suele existir si es función o constante.
            if (typeof m[simbolo] !== 'undefined') return;
        ...
        if (invalidos.length > 0) {
            return {
                valido: false,
                error: `Variable(s) no permitida(s): ${invalidos.join(', ')}. Solo se permite: ${variablesPermitidas.join(', ')}.`
            };
        }

        // Prueba de compilación final
        m.compile(exprStr);

        return { valido: true, error: null };

    } catch (err) {
        return { valido: false, error: "Error de sintaxis matemática." };
    }
};
